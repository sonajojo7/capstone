...
def create_country_dimension_table(spark, df, temp_df, output_data, mapping_file):
    ...
    country_df = spark.sql(
        ...
        SELECT 
            cv.code ,
            cv.name country_name,
            tv.average_temperature
        FROM country_codes_view cv
        LEFT JOIN average_temperature_view tv
        ON cv.name=tv.Country
        """
    ).distinct()
    ...
    country_df = spark.sql(
        """
        SELECT 
            CAST(iv.i94res AS INT) as country_code ,
            cv.country_name 
        FROM  country_view cv
        LEFT JOIN immigration_view iv
        ON iv.i94res=cv.code
        """    ).distinct()
